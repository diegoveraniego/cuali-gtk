...
def main():
    ...
    # Find wordcloud logic
    old_wc_logic = re.search(r"// --- Word Cloud Logic ---.*?GtkWidget\* create_visualizations_view", data, re.DOTALL).group(0)

    new_wc_logic = """// --- Word Cloud Logic ---
...
GtkWidget* create_visualizations_view"""
    ...
    data = data.replace(old_wc_logic, new_wc_logic)
    
    # Update refresh_visualizations for wc
    old_wc_refresh = """    if (g_wc_box) {
        populate_wordcloud(state, g_wc_box);
    }"""
    new_wc_refresh = """    if (g_wc_state) {
    ...
    data = data.replace(old_wc_refresh, new_wc_refresh)
    
    # We also need to remove g_wc_box global definition
    data = data.replace("static GtkWidget *g_wc_box = NULL;", "")
